...
import { Home, Search, Filter, Edit2, Trash2, Eye, Plus, X, Share2, Copy, MapPin, Phone, User, Building, Calendar, ExternalLink, Grid3X3, Table, Download } from 'lucide-react'
...
🏢 Magixland Real Estate`
  ...
  const filterProperties = () => {
    ...
    // Agent filter
    if (filters.agent) {
      filtered = filtered.filter((property) => property.agent_id === filters.agent)
    }
  ...
  const downloadPropertiesCSV = () => {
    ...
    const escapeCSV = (value) => {
      ...
      // Replace multiple newlines with single space for better CSV readability
      // But keep single newlines as they might be intentional
      stringValue = stringValue.replace(/\n\s*\n/g, ' ')
      
      // Always wrap in quotes for safety and proper Excel parsing
    ...
    // Format number values (no quotes for Excel to recognize as numbers)
    ...
    // Format date values (no quotes for Excel to recognize as dates)
    ...
    const csvRows = [
      ...
      ...filteredProperties.map(property => {
        // Format description - replace newlines with spaces for single-line CSV
        ...
        return [
          ...
          escapeCSV(property.project?.name || ''),
          escapeCSV(property.agent?.name || ''),
          escapeCSV(property.agent?.email || ''),
          escapeCSV(property.address || ''),
          escapeCSV(property.location_url || ''),
          escapeCSV(property.owner_details?.name || ''),
          escapeCSV(property.owner_details?.phone || ''),
          escapeCSV(property.broker_details?.name || ''),
          escapeCSV(property.broker_details?.phone || ''),
          escapeCSV(description),
          formatDateValue(property.created_at),
          formatDateValue(property.updated_at)
        ].join(',')
      })
    ]
    ...
    toast.success(`Downloaded ${filteredProperties.length} properties as CSV`)
  }
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                className="btn-secondary flex items-center justify-center px-3 sm:px-4 py-2 text-sm sm:text-base touch-manipulation"
                title="Download properties as CSV"
              >
        ...
        <div className="card mb-4 sm:mb-6 p-3 sm:p-4 lg:p-6">
          ...
          {showFilters && (
            <div className="mt-4 pt-4 border-t border-cream-dark space-y-4 sm:space-y-6">
              ...
              {/* Agent & Property Details Row */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <div>
                  <label className="block text-sm font-medium text-brown mb-2">Agent</label>
                  <select
                    value={filters.agent}
