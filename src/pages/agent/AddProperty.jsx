...
export default function AddProperty() {
  const { id } = useParams() // For copy functionality
  const { user } = useAuth()
  ...
  const fetchPropertyToCopy = async (propertyId) => {
    try {
      const { data, error } = await supabase
        .from('properties')
        .select('*')
      ...
      toast.success('Property details copied! You can now edit and save.')
    } catch (error) {
      console.error('Error fetching property to copy:', error)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-semibold text-primary-900">
            {id ? 'Copy Property' : 'Add New Property'}
          </h1>
          <p className="text-primary-600 mt-1 sm:mt-2 text-sm sm:text-base">Fill in the property details</p>
        </div>
